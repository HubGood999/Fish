...
button.Position = UDim2.new(0.5, -50, 0.5, -400)
button.Image = "rbxassetid://126344103024545"
...
uiStroke.Color = Color3.fromRGB(0, 255, 190)  -- สีเขียวอมฟ้า
...
local Tabs = {
    ...
    file = Window:AddTab({ Title = "Teleport", Icon = "file" }),
...
function Jack_Marrow()
workspace.world.npcs["Jack Marrow"].treasure.repairmap:InvokeServer()
end
function tp_Jack_Marrow()
   game.Players.LocalPlayer.Character:SetPrimaryPartCFrame(CFrame.new(-2824, 214, 1521))

end

function all_X()
local contentText = game:GetService("Players").LocalPlayer.PlayerGui["Treasure Map"].Main.CoordinatesLabel.ContentText

if not contentText:find("?") then
    Next_Treasure()
    tp_Jack_Marrow()
else
    Jack_Marrow()
...
local section = Tabs.Farm:AddSection("Chest Farm")


local Toggle = Tabs.Farm:AddToggle("AutoTridentRod", {
    Title = "Auto Open Treasure Map",
    Default = _G.Toggle,
    Description = "Enables open treasure map."
})
  Toggle:OnChanged(function(v)
    _G.all_X = v
...
spawn(function()
    ...
    -- ค้นหาหีบที่ยังไม่ได้ใช้
    local chest = nil
    for _, c in pairs(chests) do
        if not usedChests[c] then
            chest = c
            usedChests[c] = true
            break
        end
    ...
    -- ถ้าพบหีบที่ยังไม่ได้ใช้ ให้วาร์ปไปยังตำแหน่งของมัน
    if chest then
        game.Players.LocalPlayer.Character:SetPrimaryPartCFrame(chest.CFrame)
    else
        -- ถ้าหีบทั้งหมดถูกใช้ไปแล้ว ให้ล้างรายการที่ใช้แล้วและเริ่มใหม่
        usedChests = {}
    end

    wait(2) -- หน่วงเวลาเพื่อให้การวาร์ปดูเป็นธรรมชาติ
end
end)
end
end)
...
function prompt()
    ...
    if part and not part:FindFirstChild("ProximityPrompt") then
        local prompt = Instance.new("ProximityPrompt")
        prompt.HoldDuration = 0  -- Instant trigger
        prompt.MaxActivationDistance = 100  -- Activation range
        prompt.Parent = part
    end
end

local Toggle = Tabs.Farm:AddToggle("AutoTridentRod", {
    Title = "Auto Teleport To Treasure Map",
    Default = _G.tpchest_X or false,
    Description = "Enables teleport to treasure map."
})
...
Toggle:OnChanged(function(v)
    ...
    if _G.tpchest_X then
        coroutine.wrap(function()
            while _G.tpchest_X do
                pcall(function()
                    prompt()
                    E3()
                end)
...
local section = Tabs.Trading:AddSection("infinite")
...
local section = Tabs.Players:AddSection("Misc")
...
local Toggle = Tabs.Players:AddToggle("MyToggle", {
    Title = "NoClip",
    Default = false,  -- Fixed duplicate default value
    Description = "Enable character to walk through objects."
})
...
-- ฟังก์ชันเช็คตำแหน่ง Y และสร้าง Part ล่องหน
local function checkAndCreateInvisiblePart()
    if humanoidRootPart.Position.Y < 127 then
        -- สร้าง Part ใต้เท้าผู้เล่น
        local part = Instance.new("Part")
        part.Size = Vector3.new(10, 1, 10)  -- ขนาดของ Part
        part.Position = Vector3.new(humanoidRootPart.Position.X, humanoidRootPart.Position.Y - 3, humanoidRootPart.Position.Z)
        part.Anchored = true  -- ป้องกันไม่ให้ Part เคลื่อนที่
        part.Transparency = 1  -- ทำให้ล่องหน
        part.CanCollide = true  -- ปิดการชน
        part.CanTouch = false  -- ปิดการสัมผัส
        part.Parent = workspace  -- เพิ่ม Part ไปยัง workspace
    end
end
...
local Toggle = Tabs.Players:AddToggle("MyToggle", {
    Title = "Walk On Water", 
    Default = false,  -- ค่าดีฟอลต์ควรตั้งเป็น false เท่านั้นครั้งเดียว
    Description = "Enable your character to walk on water."
})
...
game:GetService("ReplicatedStorage"):WaitForChild("events"):WaitForChild("SellAll"):InvokeServer()
  ...
  local section = Tabs.Miscellaneous:AddSection("Appraiser")
...
local function CheckInventory()
    ...
    -- Find player stats in ReplicatedStorage
    local playerStats = ReplicatedStorage:FindFirstChild("playerstats")

    if playerStats then
        local playerData = playerStats:FindFirstChild(playerName)
        if playerData then
            local inventory = playerData:FindFirstChild("Inventory")
            if inventory then
                for _, item in pairs(inventory:GetChildren()) do
                    if item.Name == _G.Name_Item then
                        local weightValue = item:FindFirstChild("Weight")
                        if weightValue and weightValue.Value > tonumber(_G.Kilo_Item) then
                            _G.Appraiser_X = false
                            return
                        end
                    end
                end
...
-- Auto Appraiser function
local function Appraiser()
    workspace.world.npcs.Appraiser.appraiser.appraise:InvokeServer()
end

    local toolName =  _G.Name_Item
local player = game.Players.LocalPlayer
local backpack = player:FindFirstChild("Backpack")

-- ฟังก์ชันออโต้ถืออาวุธ
local function autoEquip()
    if backpack and backpack:FindFirstChild(toolName) then
        local tool = backpack:FindFirstChild(toolName)
        tool.Parent = player.Character -- ย้ายไอเท็มไปที่ตัวละคร
    end
end


-- Continuously check the inventory and appraise items
spawn(function()
    while task.wait(0.3) do
        pcall(function()
            if _G.Appraiser_X then
                CheckInventory()
                Appraiser()
                autoEquip()
            end
...
-- UI Inputs
local JobIDInput = Tabs.Miscellaneous:AddInput("JobIDInput", {
    Title = "Enter Kilo Item [ Not working❌ ]",
    Default = "",
    Placeholder = "Kilo item.",
    Numeric = false,
    Finished = false,
    Callback = function(Value)
        _G.Kilo_Item = Value
...
local NameInput = Tabs.Miscellaneous:AddInput("NameInput", {
    Title = "Enter Name Item  [ Not working❌ ]",
    Default = "",
...
local Toggle = Tabs.Miscellaneous:AddToggle("MyToggle", {
    Title = "Auto Appraiser [ Not working❌ ]", 
    Default = false, 
    Description = "Appraiser Auto Kilo Item."
})
...
Tabs.Miscellaneous:AddButton({
    Title = "Print Name Item To Console",
    Description = "Print Item check name item.",
    Callback = function()
 local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")

-- ดึงชื่อของผู้เล่นที่กำลังรันสคริปต์
local player = Players.LocalPlayer
local playerName = player.Name

-- ค้นหาข้อมูลใน ReplicatedStorage
local playerStats = ReplicatedStorage:FindFirstChild("playerstats")
...
if playerStats then
    ...
    if playerData then
        ...
        if inventory then
            ...
            -- วนลูปไอเทมทั้งหมดและเพิ่มลงในตาราง
            for _, item in pairs(inventory:GetChildren()) do
                local weightValue = item:FindFirstChild("Weight")
                if weightValue and weightValue:IsA("ValueBase") then
                    table.insert(itemsTable, {name = item.Name, weight = weightValue.Value})
                end
            end

            -- เรียงลำดับตารางจากค่ามากไปน้อย
            table.sort(itemsTable, function(a, b)
                return a.weight > b.weight
            end)

            -- แสดงผลรายการที่เรียงลำดับแล้ว
            for _, itemData in ipairs(itemsTable) do
                print("-----------------------------------------------")
                print("Item:", itemData.name, "- kilo:", itemData.weight)
            end
            print("📢📢📢📢📢📢📢📢📢END📢📢📢📢📢📢📢📢📢")
        end
    end
...
Tabs.Miscellaneous:AddButton({
    Title = "Open Console",
    Description = "Open console to check item names.",
    Callback = function()
...
local section = Tabs.file:AddSection("Island")
local TeleportDropdown = Tabs.file:AddDropdown("Dropdown", {
...
Tabs.file:AddButton({
    ...
    Callback = function()
        local teleportPositions = {
            ...
            ["Snowcapv"] = CFrame.new(2599.12769, 135.283859, 2414.37671, 0.802532911, -8.18404118e-08, 0.596607864, 5.14819192e-08, 1, 6.7924816e-08, -0.596607864, -2.37973818e-08, 0.802532911),
            ...
            ["Roslitv"] = CFrame.new(-1446.406, 134.9541, 702.1841), --ใช้ได้
            ...
            ["Desolatev"] = CFrame.new(-1657.3175, -214.182587, -2844.28882, 1, -1.77927966e-08, -2.00143298e-14, 1.77927966e-08, 1, 7.70572655e-08, 1.86432663e-14, -7.70572655e-08, 1),
...
local section = Tabs.file:AddSection("File Teleport")
